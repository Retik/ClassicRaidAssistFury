...
local function OnValueChanged(spellName)
    if(ClassicRaidAssistFury.db.char.buffsToCheckFor[spellName] == false or ClassicRaidAssistFury.db.char.buffsToCheckFor[spellName] == nil) then
        ClassicRaidAssistFury.db.char.buffsToCheckFor[spellName] = true
    else
        ClassicRaidAssistFury.db.char.buffsToCheckFor[spellName] = false
    end
end
...
function RaidBuffsTab:ScanRaidBuffs()
    ...
    for i = 1, MAX_RAID_MEMBERS do
        ...
        if(tempPlayerName ~= nil and class ~= nil) then
            ...
            for key, value in pairs(ClassicRaidAssistFury.db.char.buffsToCheckFor) do
                ...
                if value then
                    ...
                    for k, v in pairs(raidBuffNames[key]["alts"]) do
                        ...
                        if (ClassicRaidAssistFury.db.char.smartBuffFiltering and (englishClass == "WARRIOR" or englishClass == "ROGUE") and (raidBuffNames[key]['smartFiltering'])) then
                ...
                if type(v) ~= "boolean" then
                    ...
                    if ( string.len(stringCompose) + string.len(stringToAdd) ) >= 255 then
                        if(sendBeginningMessage) then
                            ...
                            CRA_SendChatMessage("[Classic Raid Assist Fury] Outputting Missing Raid Buffs: ", channel)
                        end
...
function RaidBuffsTab:DrawRaidBuffs(container)
    ...
    reportBtn:SetCallback("OnClick", function()
        RaidBuffsTab:outputMissingDebuff(ClassicRaidAssistFury.db.char.CurrentRaidBuffsOutputChannel)
    end)
    ...
    reportDropdown:SetValue(ClassicRaidAssistFury.db.char.CurrentRaidBuffsOutputChannel)
    reportDropdown:SetCallback("OnValueChanged", function(widget, event, key)
        ClassicRaidAssistFury.db.char.CurrentRaidBuffsOutputChannel = key
    end)
