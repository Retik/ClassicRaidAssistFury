...
local function OnValueChanged(spellName)
    if(ClassicRaidAssistFury.db.char.worldBuffsToCheckFor[spellName] == false or ClassicRaidAssistFury.db.char.worldBuffsToCheckFor[spellName] == nil) then
        ClassicRaidAssistFury.db.char.worldBuffsToCheckFor[spellName] = true
    else
        ClassicRaidAssistFury.db.char.worldBuffsToCheckFor[spellName] = false
    end
end
...
function WorldBuffsTab:ScanWorldBuffs()
    ...
    for i = 1, MAX_RAID_MEMBERS do
        ...
        if(tempPlayerName ~= nil and class ~= nil) then
            ...
            for key, value in pairs(ClassicRaidAssistFury.db.char.worldBuffsToCheckFor) do
                ...
                if type(v) ~= "boolean" then
                    ...
                    if ( string.len(stringCompose) + string.len(stringToAdd) ) >= 255 then
                        if(sendBeginningMessage) then
                            ...
                            CRA_SendChatMessage("[Classic Raid Assist Fury] Outputting People With World Buffs: ", channel)
                        end
...
function WorldBuffsTab:DrawWorldBuffs(container)
    ...
    if (ClassicRaidAssistFury.faction == "Horde") then
        ...
        rallyingCry:SetValue(ClassicRaidAssistFury.db.char.worldBuffsToCheckFor[spellName])
        ...
        songFlower:SetLabel("")
        songFlower:SetValue(ClassicRaidAssistFury.db.char.worldBuffsToCheckFor[spellName])
        ...
        warchiefsBlessing:SetValue(ClassicRaidAssistFury.db.char.worldBuffsToCheckFor[spellName])
        ...
        dmf:SetWidth(65)
        dmf:SetLabel("")
        dmf:SetValue(ClassicRaidAssistFury.db.char.worldBuffsToCheckFor[spellName])
        ...
        dmBuffs:SetValue(ClassicRaidAssistFury.db.char.worldBuffsToCheckFor[spellName])
    ...
    reportBtn:SetCallback("OnClick", function()
        WorldBuffsTab:outputBuffs(ClassicRaidAssistFury.db.char.CurrentWorldBuffsOutputChannel)
    end)
    ...
    reportDropdown:SetValue(ClassicRaidAssistFury.db.char.CurrentWorldBuffsOutputChannel)
    reportDropdown:SetCallback("OnValueChanged", function(widget, event, key)
        ClassicRaidAssistFury.db.char.CurrentWorldBuffsOutputChannel = key
    end)
