...
RaidItemsTab.raidItems = {
    ["Aqual Quintessence"] = {
        ["itemIds"] = {"22754", "17333"},
        ["alts"] = {"Eternal Quintessence", "Aqual Quintessence"},
        ["dropdownName"] = "Eternal/Aqual Quintessence",
...
local function ScanRaidItems()
    ...
    for key, value in pairs(subgroups) do
        ...
        for k, v in pairs(value) do
            ...
            if(RaidItemsTab.raidItems[ClassicRaidAssistFury.db.char.CurrentRaidItemCheck]['hasThisItem'][k] == nil) then
            ...
            elseif(RaidItemsTab.raidItems[ClassicRaidAssistFury.db.char.CurrentRaidItemCheck]['hasThisItem'][k] == false) then
                local currSpell = AceGUI:Create("Icon")
...
local function outPutItems(channel)
    local initialOutput = "[Classic Raid Assist Fury] Outputting People without or having an unknown status for "

    -- local beginningString = "Missing item "
    for k, v in pairs(RaidItemsTab.raidItems[ClassicRaidAssistFury.db.char.CurrentRaidItemCheck]['alts']) do
    ...
    for i = 1, MAX_RAID_MEMBERS do
        ...
        if(tempPlayerName ~= nil and class ~= nil) then
            ...
            if(RaidItemsTab.raidItems[ClassicRaidAssistFury.db.char.CurrentRaidItemCheck]['hasThisItem'][currPlayerName] == nil or RaidItemsTab.raidItems[ClassicRaidAssistFury.db.char.CurrentRaidItemCheck]['hasThisItem'][currPlayerName] == false) then
...
function RaidItemsTab:DrawRaidItems(container)
    ...
    itemDropdown:SetValue(ClassicRaidAssistFury.db.char.CurrentRaidItemCheck)
    itemDropdown:SetCallback("OnValueChanged", function(widget, event, key)
        ClassicRaidAssistFury.db.char.CurrentRaidItemCheck = key
        ClassicRaidAssistFuryGUI:ReloadTab()
    end)
    ...
    scanBtn:SetCallback("OnClick", function()
        ...
        -- for key, value in pairs(RaidItemsTab.raidItems[ClassicRaidAssistFury.db.char.CurrentRaidItemCheck]['itemIds']) do
        --     -- print(value)
        ...
        --     if (count > 0) then
        --         -- print(count)
        --         RaidItemsTab.raidItems[ClassicRaidAssistFury.db.char.CurrentRaidItemCheck]['hasThisItem'][currPlayerName] = true
        --         -- print(currPlayerName)
        --         break
        --     else 
        --         RaidItemsTab.raidItems[ClassicRaidAssistFury.db.char.CurrentRaidItemCheck]['hasThisItem'][currPlayerName] = false
        --     end
        -- end

        RaidItemsTab.raidItems[ClassicRaidAssistFury.db.char.CurrentRaidItemCheck]['hasThisItem'] = {}
        ClassicRaidAssistFury:ScanForItem(ClassicRaidAssistFury.db.char.CurrentRaidItemCheck)
        ClassicRaidAssistFury.db.char.raidItemsContainer = container
    ...
    reportBtn:SetCallback("OnClick", function()
        outPutItems(ClassicRaidAssistFury.db.char.CurrentRaidItemsOutputChannel)
    end)
    ...
    reportDropdown:SetValue(ClassicRaidAssistFury.db.char.CurrentRaidItemsOutputChannel)
    reportDropdown:SetCallback("OnValueChanged", function(widget, event, key)
        ClassicRaidAssistFury.db.char.CurrentRaidItemsOutputChannel = key
    end)
