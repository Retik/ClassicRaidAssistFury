...
local function OnValueChanged(spellName)
    if(ClassicRaidAssistFury.db.char.paladinBuffsToCheckFor[spellName] == false or ClassicRaidAssistFury.db.char.paladinBuffsToCheckFor[spellName] == nil) then
        ClassicRaidAssistFury.db.char.paladinBuffsToCheckFor[spellName] = true
    else
        ClassicRaidAssistFury.db.char.paladinBuffsToCheckFor[spellName] = false
    end
end
...
function PaladinBuffsTab:ScanPaladinBuffs()
    ...
    for i = 1, MAX_RAID_MEMBERS do
        ...
        if(tempPlayerName ~= nil and class ~= nil) then
            ...
            for key, value in pairs(ClassicRaidAssistFury.db.char.paladinBuffsToCheckFor) do
                ...
                if value then
                    ...
                    for k, v in pairs(paladinBuffNames[key]["alts"]) do
                        ...
                        if (ClassicRaidAssistFury.db.char.smartBuffFiltering and (englishClass == "WARRIOR" or englishClass == "ROGUE") and (paladinBuffNames[key]['smartFiltering']) and key == "Greater Blessing of Wisdom") then
                            subgroups[subgroup][currPlayerName]['spellsMissing'][key] = {['missingBuff'] = 0}
                        elseif (ClassicRaidAssistFury.db.char.smartBuffFiltering and (englishClass == "MAGE" or englishClass == "WARLOCK" or englishClass == "PRIEST") and (paladinBuffNames[key]['smartFiltering']) and key == "Greater Blessing of Might") then
                            subgroups[subgroup][currPlayerName]['spellsMissing'][key] = {['missingBuff'] = 0}
                        else 
                            if not hasBuff then
                ...
                if type(v) ~= "boolean" then
                    ...
                    if ( string.len(stringCompose) + string.len(stringToAdd) ) >= 255 then
                        if(sendBeginningMessage) then
                            ...
                            CRA_SendChatMessage("[Classic Raid Assist Fury] Outputting Missing Paladin Buffs: ", channel)
                        end
...
function PaladinBuffsTab:DrawPaladinBuffs(container)
    ...
    bok:SetWidth(60)
    bok:SetLabel("")
    bok:SetValue(ClassicRaidAssistFury.db.char.paladinBuffsToCheckFor[spellName])
    ...
    bom:SetWidth(60)
    bom:SetLabel("")
    bom:SetValue(ClassicRaidAssistFury.db.char.paladinBuffsToCheckFor[spellName])
    ...
    bow:SetWidth(60)
    bow:SetLabel("")
    bow:SetValue(ClassicRaidAssistFury.db.char.paladinBuffsToCheckFor[spellName])
    ...
    bosalv:SetWidth(60)
    bosalv:SetLabel("")
    bosalv:SetValue(ClassicRaidAssistFury.db.char.paladinBuffsToCheckFor[spellName])
    ...
    bosanc:SetWidth(60)
    bosanc:SetLabel("")
    bosanc:SetValue(ClassicRaidAssistFury.db.char.paladinBuffsToCheckFor[spellName])
    ...
    bol:SetWidth(60)
    bol:SetLabel("")
    bol:SetValue(ClassicRaidAssistFury.db.char.paladinBuffsToCheckFor[spellName])
    ...
    reportBtn:SetCallback("OnClick", function()
        PaladinBuffsTab:outputMissingDebuff(ClassicRaidAssistFury.db.char.CurrentPaladinBuffsOutputChannel)
    end)
    ...
    reportDropdown:SetValue(ClassicRaidAssistFury.db.char.CurrentPaladinBuffsOutputChannel)
    reportDropdown:SetCallback("OnValueChanged", function(widget, event, key)
        ClassicRaidAssistFury.db.char.CurrentPaladinBuffsOutputChannel = key
    end)
